...
pub(super) fn queue_get_songs() -> Result<Vec<Song>, Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "queue_get_songs", FailureType::Fetal)))?;
    rx.recv()
    .map_err(|e| Failure::from((e.into(), "queue_get_songs", FailureType::Fetal)))
}
...
//     .map_err(|e| Failure::from((e.into(), "playlist_get_name", FailureType::Fetal)))?;
//     rx.recv()
//     .map_err(|e| Failure::from((e.into(), "playlist_get_name", FailureType::Fetal)))
// }
...
pub(super) fn playlist_get_songs(playlist: &str) -> Result<Vec<Song>, Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_get_songs", FailureType::Fetal)))?;
    rx.recv()
    .map_err(|e| Failure::from((e.into(), "playlist_get_songs", FailureType::Fetal)))
}
...
pub(super) fn playlist_get_song(playlist: &str, index: usize) -> Result<Song, Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_get_song", FailureType::Fetal)))?;
    rx.recv()
    .map_err(|e| Failure::from((e.into(), "playlist_get_song", FailureType::Fetal)))
}
...
pub(super) fn queue_next() -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "queue_next", FailureType::Fetal)))
}

pub(super) fn current_song() -> Result<Song, Failure> {
    let (tx, rx) = mpsc::channel();
    MAIN_SENDER.get().unwrap().clone().send(Command::CurrentSong(tx))
    .map_err(|e| Failure::from((e.into(), "current_song", FailureType::Fetal)))?;
    rx.recv()
    .map_err(|e| Failure::from((e.into(), "current_song", FailureType::Fetal)))
}
...
pub(super) fn play_new(song: Song) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "play_new", FailureType::Fetal)))
}

pub(super) fn queue_add(song: Song) -> Result<(), Failure> {
    MAIN_SENDER.get().unwrap().clone().send(Command::QueueAdd(song))
    .map_err(|e| Failure::from((e.into(), "queue_add", FailureType::Fetal)))
}
...
pub(super) fn queue_remove(song_index: usize) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "queue_remove", FailureType::Fetal)))
}
...
pub(super) fn queue_playlist(playlist: &str) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "queue_playlist", FailureType::Fetal)))
}

// pub(super) fn queue_current_playlist() -> Result<(), Failure> {
//     MAIN_SENDER.get().unwrap().clone().send(Command::QueueCurrentPlaylist)
//     .map_err(|e| Failure::from((e.into(), "queue_current_playlist", FailureType::Fetal)))
// }
...
pub(super) fn playlist_add(playlist: &str, song: Song) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_add", FailureType::Fetal)))
}
...
pub(super) fn playlist_remove(playlist: &str, index: usize) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_remove", FailureType::Fetal)))
}

// pub(super) fn playlist_load(name: &str) -> Result<(), Failure> {
//     MAIN_SENDER.get().unwrap().clone().send(Command::PlaylistLoad(name.to_string()))
//     .map_err(|e| Failure::from((e.into(), "playlist_load", FailureType::Fetal)))
// }
...
pub(super) fn playlist_move_song(playlist: &str, from: usize, to: usize) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_move_song", FailureType::Fetal)))
}
...
pub(super) fn playlist_new(name: &str, external_type: Option<ExternalType>) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_new", FailureType::Fetal)))
}
...
pub(super) fn playlist_set_name(playlist: &str, name: &str) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_set_name", FailureType::Fetal)))
}
...
pub(super) fn playlist_copy_to(from: &str, to: &str) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_copy_to", FailureType::Fetal)))
}
...
pub(super) fn playlist_add_playlist(from: &str, to: &str) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_add_playlist", FailureType::Fetal)))
}
...
pub(super) fn playlist_clear(playlist: &str) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "playlist_clear", FailureType::Fetal)))
}

pub(super) fn shutdown() -> Result<(), Failure> {
    MAIN_SENDER.get().unwrap().clone().send(Command::Shutdown)
    .map_err(|e| Failure::from((e.into(), "shutdown", FailureType::Fetal)))
}

pub(super) fn song_duration() -> Result<Duration, Failure> {
    let (tx, rx) = mpsc::channel();
    MAIN_SENDER.get().unwrap().clone().send(Command::SongDuration(tx))
    .map_err(|e| Failure::from((e.into(), "song_duration", FailureType::Fetal)))?;
    match rx.recv() {
        Ok(progress) => progress,
        Err(e) => Err(Failure::from((e.into(), "song_duration", FailureType::Fetal))),
    }
}
...
pub(super) fn song_duration_gone() -> Result<Duration, Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "song_duration_gone", FailureType::Fetal)))?;
    match rx.recv() {
        Ok(progress) => progress,
        Err(e) => Err(Failure::from((e.into(), "song_duration_gone", FailureType::Fetal))),
    }
}

pub(super) fn queue_shuffle() -> Result<(), Failure> {
    MAIN_SENDER.get().unwrap().clone().send(Command::QueueShuffle)
    .map_err(|e| Failure::from((e.into(), "queue_shuffle", FailureType::Fetal)))
}

pub(super) fn queue_clear() -> Result<(), Failure> {
    MAIN_SENDER.get().unwrap().clone().send(Command::QueueClear)
    .map_err(|e| Failure::from((e.into(), "queue_clear", FailureType::Fetal)))
}

pub(super) fn connect_to_server() -> Result<(), Failure> {
    MAIN_SENDER.get().unwrap().clone().send(Command::ServerConnect)
    .map_err(|e| Failure::from((e.into(), "connect_to_server", FailureType::Fetal)))
}
...
pub(super) fn send_message_to_server(message: &str) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "send_message_to_server", FailureType::Fetal)))
}
...
pub(super) fn add_server(name: String, address: String, certificate_path: String) -> Result<(), Failure> {
    ...
    .map_err(|e| Failure::from((e.into(), "add_server", FailureType::Fetal)))
}