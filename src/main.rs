...
fn startup(transmit: std::sync::mpsc::Sender<crate::Command>) -> Result<Internal, String> {
    ...
    let content = match std::fs::read_to_string(format!("{}config.toml", CONFIG_FOLDER)) {
        ...
        Err(_) => {
            ...
            toml::to_string(&default)
                .map_err(|e| format!("Failed to make default config: {}", e))?;
            Err(format!(
    ...
    //read config
    let config: Config =
        toml::from_str(&content).map_err(|e| format!("Failed to read config file: {}", e))?;

    // Set DATA_FOLDER
    DATA_FOLDER
        .set(config.data_folder.clone())
        .map_err(|_| "DATA_FOLDER already set".to_string())?;

    // Set LOCAL_SONG_FOLDER_PATH if provided in config
    if let Some(local_path) = config.local_song_folder_path {
        LOCAL_SONG_FOLDER_PATH
            .set(local_path)
            .map_err(|_| "LOCAL_SONG_FOLDER_PATH already set".to_string())?;
    }
    ...
    if !startup_data.last_shutdown_clean {
        println!(
            "Warning: Last shutdown was not clean, data may be corrupted, lost or incorrect. \n Attempting to continue... "
        );
    }
    ...
    let playlist = if data_folder
        .join("playlists")
        .join(format!("{}.json", startup_data.last_played_playlist))
        .exists()
    {
...
impl StartupData {
    ...
    fn save(&self) -> Result<(), String> {
        ...
        match std::fs::write(
            format!(
                "{}startup.toml",
                DATA_FOLDER.get().ok_or("DATA_FOLDER not set".to_string())?
            ),
            toml::to_string(self)
                .map_err(|e| format!("Failed to serialize startup data: {}", e))?,
        ) {
